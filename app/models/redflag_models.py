...
class Redflag:
    ...
    def update_redflag_comment(self, id, comment):
        sql = "UPDATE incidents SET comment='"+comment+"' WHERE incident_type='red-flag' and id='{}'\
               ".format(id)
    ...
    def update_redflag_location(self, id, location):
        sql = "UPDATE incidents SET location='"+location+"' WHERE incident_type='red-flag' and id='{}'\
                ".format(id)
        db_content.cur.execute(sql)
        return jsonify({"status": 200, "data": [{"message": "Updated red-flag\
    ...
    def get_red_flag(self, id):
        db_content.dict_cursor.execute("SELECT * from incidents WHERE incident_type='red-flag' and id='{}'\
                                       ".format(id))
        data = db_content.dict_cursor.fetchall()
        return jsonify({"status": 200, "data": data}), 200

    def delete_redflag(self, id):
        sql = "DELETE from incidents WHERE incident_type='red-flag' and id='{}'\
        ".format(id)
        db_content.cur.execute(sql)
